...
function getNotificationIcon(type: string) {
  switch (type) {
    ...
    case "commitPushed":
      return <GitCommit className="h-4 w-4 text-accent" />;
    case "performanceScored":
...
function getNavigationTarget(type: string): string | undefined {
  switch (type) {
    case "messageReceived":
      return "messages";
    case "projectAssigned":
...
export default function NotificationCenter({
  ...
  const [notifications, setNotifications] = useState<View__5[]>([]);
  ...
  const handleNotifClick = (notif: View__5) => {
    if (!notif.isRead) handleMarkRead(notif.id);
    const target = getNavigationTarget(notif.notificationType);
    ...
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        ...
        >
          ...
          {unreadCount > 0 && (
            <span
              className="absolute -top-0.5 -right-0.5 h-4 min-w-4 px-1 text-[9px] bg-destructive text-destructive-foreground rounded-full flex items-center justify-center font-bold"
